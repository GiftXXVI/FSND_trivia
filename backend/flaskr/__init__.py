...
def create_app(test_config=None):
  ...
  '''
    ...
    def get_questions():
        ...
        categories = Question.query.order_by(Question.category, Question.id).with_entities(Question.category).distinct().all()
        lcategory = [item.category for item in categories]
        if len(questions) == 0:
            return abort(404)
        else:
            prepared_questions = prepare_questions(request, questions, lcategory)
  ...
  '''
    ...
    def delete_question(int question_id):
  ...
  '''
    ...
    def search_posts():
      ...
      if body is None:
        ...
        try:
          questions = Questions.query.filter_by(Question.question.ilike(search_term)).order_by(Question.category, Question.id).all()
          categories = Question.query.filter_by(Question.question.ilike(search_term)).order_by(Question.category, Question.id).with_entities(Question.category).distinct().all()
          lcategory = [item.category for item in categories]
          if len(questions) == 0:
            return abort(404)
          else:
  ...
  '''
    ...
    def get_cat_questions(category_id)):
        questions = Question.query.filter_by(Question.category==category_id).order_by(Question.category, Question.id).all()
        categories = Question.query.filter_by(Question.category==category_id).order_by(Question.category, Question.id).with_entities(Question.category).distinct().all()
        lcategory = [item.category for item in categories]
        if len(questions) == 0:
          return abort(404)
        else:
